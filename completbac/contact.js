...
router.post("/contact_mekody", async (req, res) => {
    try {
        ...
        const result = await pool.query(
            "INSERT INTO contact_mekody (nom_complet, courriel, entreprise, service_interesse, message) VALUES ($1, $2, $3, $4, $5) RETURNING *",
...
router.get("/contact_mekody/:id", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM contact_mekody WHERE id = $1", [
            req.params.id,
        ]);
...
router.put("/contact_mekody/:id", async (req, res) => {
    try {
        const { nom_complet, courriel, entreprise, service_interesse, message } = req.body;
        const result = await pool.query(
            "UPDATE contact_mekody SET nom_complet = $1, courriel = $2, entreprise = $3, service_interesse = $4, message = $5 WHERE id = $6 RETURNING *",
...
router.delete("/contact_mekody/:id", async (req, res) => {
    try {
        const result = await pool.query("DELETE FROM contact_mekody WHERE id = $1 RETURNING *", [req.params.id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: "Contact non trouvé" });
        }
        res.json({ message: "Contact supprimé ✅", deletedContact: result.rows[0] });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
