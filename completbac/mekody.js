...
router.post("/contact_mekody", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "INSERT INTO contact_mekody (nom, email, message) VALUES ($1, $2, $3) RETURNING *",
      [nom, email, message]
    );
    res.json(result.rows[0]);
  } catch (err) {
...
router.get("/contact_mekody", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM contact_mekody");
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put("/contact_mekody/:id", async (req, res) => {
  try {
    const { nom, email, message } = req.body;
    const result = await pool.query(
      "UPDATE contact_mekody SET nom = $1, email = $2, message = $3 WHERE id = $4 RETURNING *",
      [nom, email, message, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.delete("/contact_mekody/:id", async (req, res) => {
  try {
    await pool.query("DELETE FROM contact_mekody WHERE id = $1", [req.params.id]);
